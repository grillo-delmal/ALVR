...
use alvr_session::{ClientsideFoveationMode, Settings};
...
pub struct SessionRunningContext {
    ...
    session_running: Arc<RelaxedAtomic>,
...
impl Drop for SessionRunningContext {
    fn drop(&mut self) {
...
fn input_pipeline(xr_ctx: &XrContext, input_ctx: &mut InputContext) {
    // Streaming related inputs are updated here. Make sure every input poll is done in this
    // thread
    if let Err(e) = xr_ctx
        .session
        .sync_actions(&[(&input_ctx.interaction_context.action_set).into()])
    {
        error!("{e}");
        return;
    }
    ...
    'head_tracking: {
        let Ok((view_flags, views)) = xr_ctx.session.locate_views(
            ...
            &input_ctx.reference_space.read(),
        ) else {
    ...
    let (left_hand_motion, left_hand_skeleton) = interaction::get_hand_motion(
        ...
        &input_ctx.reference_space.read(),
        tracker_time,
        &input_ctx.interaction_context.hands_interaction[0],
        &mut input_ctx.last_hand_positions[0],
    );
    let (right_hand_motion, right_hand_skeleton) = interaction::get_hand_motion(
        &xr_ctx.session,
        &input_ctx.reference_space.read(),
        tracker_time,
        &input_ctx.interaction_context.hands_interaction[1],
        &mut input_ctx.last_hand_positions[1],
    );
    ...
    let face_data = FaceData {
        eye_gazes: interaction::get_eye_gazes(
            &xr_ctx.session,
            &input_ctx.interaction_context.face_sources,
            &input_ctx.reference_space.read(),
            to_xr_time(now),
        ),
        ...
        htc_eye_expression: interaction::get_htc_eye_expression(
            &input_ctx.interaction_context.face_sources,
        ),
        htc_lip_expression: interaction::get_htc_lip_expression(
            &input_ctx.interaction_context.face_sources,
        ),
    };
    ...
    let button_entries = interaction::update_buttons(
        &xr_ctx.session,
        &input_ctx.interaction_context.button_actions,
    );
...
fn initialize_stream(
    ...
    #[cfg(target_os = "android")]
    {
        if let alvr_common::settings_schema::Switch::Enabled(config) =
            config.settings.headset.face_tracking
        {
    ...
    let foveation_profile =
        if let Some(config) = config.settings.video.clientside_foveation.into_option() {
    ...
    alvr_client_core::opengl::start_stream(
        ...
        config.settings.video.foveated_rendering.into_option(),
...
pub fn entry_point() {
    ...
    'session_loop: loop {
        ...
        let interaction_context = Arc::new(interaction::initialize_interaction(
            ...
            stream_config
                .as_ref()
                .and_then(|c| c.settings.headset.face_tracking.as_option())
                ...
                match event {
                    ...
                    xr::Event::SessionStateChanged(event) => match event.state() {
                        xr::SessionState::READY => {
                            ...
                            let session_running = Arc::new(RelaxedAtomic::new(true));
                            ...
                            let input_rate = stream_config
                                .as_ref()
                                .map(|c| c.refresh_rate_hint)
                                .unwrap_or(60.0);
                            let mut input_context = InputContext {
                                interaction_context: Arc::clone(&interaction_context),
                                views_history_sender: views_history_sender.clone(),
                            ...
                            let input_thread = thread::spawn({
                                ...
                                move || {
                                    ...
                                    while session_running.value() {
                                        input_pipeline(&xr_ctx, &mut input_context);
                            ...
                            let stream_context = stream_config.take().map(|config| {
                                initialize_stream(&xr_ctx, &interaction_context, platform, config)
                            });
